 ...
 */
dbus_bool_t
kdbus_connection_get_unix_user(DBusConnection *connection,
                               const char     *name,
                               unsigned long  *uid,
                               DBusError      *error)
{
 ...
 */
dbus_bool_t
kdbus_connection_get_unix_process_id(DBusConnection *connection,
                                     const char     *name,
                                     unsigned long  *pid,
                                     DBusError      *error)
{
  ...
  if(inter_ret == 0) //name found
    ...
    dbus_set_error (error, DBUS_ERROR_NAME_HAS_NO_OWNER, "Could not get PID of name '%s': no such name", name);
  else
    {
